...
os.system("stty -F /dev/ttyS0 ispeed 9600")
os.system('sudo gpsd /dev/ttyS0 -G -n -F /var/run/gpsd.sock')
gpsd.connect()
global Ht
global turn
...
thread=Thread(target = gpsmonitor)
thread.start()
#
...
while True:
        ...
        try:
                ...
                print ("\tAx=%.2f g" %Ax, "\tAy=%.2f g" %Ay, "\tAz=%.2f g" %Az, "\tAt=%.2f g" %Ta,"\tLa=" ,La,"\tLo=" ,Lo,"\tHt=" ,turn)
                i=i+1
                now = datetime.now()
                ...
                jsonData = str({"Ax":Ax,"Ay":Ay,"Az":Az,"Total acceleration":Ta,"Latitude":La,"Longitude":Lo,"Ht":turn})
